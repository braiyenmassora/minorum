...
const serverEnvSchema = z.object({
  ...
  MINORUM_SEARCH_PROVIDER: z.string().optional(),
  MINORUM_FETCH_PROVIDER: z.string().optional(),
});
...
function readServerEnv(): ServerEnv {
  return serverEnvSchema.parse({
    ...
    MINORUM_SEARCH_PROVIDER:
      process.env.MINORUM_SEARCH_PROVIDER?.trim() || undefined,
    MINORUM_FETCH_PROVIDER:
      process.env.MINORUM_FETCH_PROVIDER?.trim() || undefined,
  });
}
...
export function readWebToolsConfigFromEnv(): import("@/lib/core/config/web-tools-config").WebToolsConfig {
  ...
  const modelAllowlist = modelsRaw
    ...
    : [];
  const env = readServerEnv();
  const searchProvider = env.MINORUM_SEARCH_PROVIDER || "search-combo";
  const fetchProvider = env.MINORUM_FETCH_PROVIDER || "fetch-combo";
